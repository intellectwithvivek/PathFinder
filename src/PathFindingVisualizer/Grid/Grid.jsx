...
export default class Grid extends Component {
  ...
  setSpeed(speed) {
    ...
    if (this.isVisualizing) {
      if (this.timeoutVisitedNodesInOrder.length) {
        for (let timeout of this.timeoutVisitedNodesInOrder)
          clearTimeout(timeout);
        for (let timeout of this.timeoutNodesInShortestPathOrder)
          clearTimeout(timeout);
  ...
  animate(
    ...
    this.timeoutVisualization = null;
    for (
      let i = visitedNodesInOrderIndex;
      i < visitedNodesInOrder.length;
      i++
    ) {
      const node = visitedNodesInOrder[i];
      this.timeoutVisitedNodesInOrder.push(
        setTimeout(this.setIsVisited(node), this.speed * k++)
      );
    }
    for (
      let i = nodesInShortestPathOrderIndex;
      i < nodesInShortestPathOrder.length;
      i++
    ) {
      const node = nodesInShortestPathOrder[i];
      this.timeoutNodesInShortestPathOrder.push(
  ...
  animateMaze(walls, index) {
    ...
    for (let i = index; i < walls.length; i++) {
      const wall = walls[i];
      this.timeoutMazeNodes.push(
        setTimeout(this.setIsIncludedInMaze(wall), this.speed * k++)
      );
    }
    this.timeoutVisualization = setTimeout(
      this.visualizationStopped,
